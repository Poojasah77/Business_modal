...
const BusinessAddModal = ({modalIsOpen, setModalIsOpen, handleAddBusiness}) => {
  ...
  return (
    <>
    <div onClick={closeModal} className={`inset-0 absolute h-screen w-screen flex justify-center items-center transition-all ${modalIsOpen? 'bg-black/60 visible ':"invisible"}`}>
            <div onClick={(e => e.stopPropagation())} className={`h-[90%] w-1/3 bg-white rounded-lg py-4 px-8 shadow-lg ${modalIsOpen ? "translate-y-0": "translate-y-[900px]" } ${modalIsOpen ? "scale-100": "scale-0" }`}>
              <div className="flex flex-row justify-between items-center">
                <span className="text-lg font-bold">Add Business</span>
                <GiTireIronCross  size={20} color="gray" onClick={closeModal} />
              </div>
              ...
              <div className="overflow-hidden w-full h-1/3 mt-2">
              ...
              <div className="flex flex-row justify-end gap-4 my-4">
                <button  onClick={closeModal} className="text-rose-500 text-bold border border-rose-500 py-2 px-4 rounded-md hover:text-rose-500/60 hover:border-rose-500/60">Close</button>
